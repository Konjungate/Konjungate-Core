...
void OverviewPage::updateWatchOnlyLabels(bool showWatchOnly)
{
    ...
    //ui->labelKONJtotalimg->setVisible(showWatchOnly);    // show watch-only balance separator line
    ...
    if(stat)
    {
        ...
        if(nSyncPercentage != 100)
        {
            if(clientModel->getNumConnections() > 0)
                ui->clStat->setText("<font color=#c8c8c8>" + QSyncing + "</font>"); // Color !Not Syncing
        }
        return;
    }
}
...
{
    KonjungateGUI::gotoHistoryPage_static();
}
